...
export default function test() {
  // 자동 완성 리스트 샘플 
  const pokemons = [{"name":"test12","engname":"pikachu"},{"name":"test13"},{"name":"test14"},{"name":"est33"},{"name":"estt15"},{"name":"estttttt12444"}]
    ...
    <main className="min-h-screen flex flex-col items-center justify-between p-24 bg-gray-800">
      <div className="max-w-4xl w-full space-x-6 items-center border-b justify-center font-mono text-sm flex border-b border-gray-300 dark:from-inherit rounded-xl border bg-gray-200 p-8 ">
        <div className="w-full space-x-6 items-center border-b justify-center font-mono text-sm flex">
          <img class="h-auto rounded-full mx-auto" src="/test.png" alt="" width="50" height="24"></img>
          <div className="w-full relative ">
            ...
            <ul className="absolute top-14 w-full top-0 bg-white p-4" hidden={isHidden}>
            {pokemons.map((pokemon, idx) => (
              <li 
              className="border-b p-1.5 font-sans list" 
              key={idx} 
              hidden={!pokemon.name.substring(0,inputVal.length).includes(search)} 
              onMouseOver={(e)=> autocomplete_keyevent("MouseOver",e.target)} 
